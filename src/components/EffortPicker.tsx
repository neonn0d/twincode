...
export function EffortPicker({ onSelect, onCancel }: Props) {
  ...
  function handleSelect(value: string) {
    if (value === 'auto') {
      ...
      // Normalize OpenAI-shaped 'xhigh' to standard 'max' so AppState +
      // settings.json always hold a persistable value. The shim converts back
      // to 'xhigh' at the request boundary.
  ...
  // For OpenAI/Codex: prefer user's current selection (max → xhigh for option
  // matching), otherwise model alias default, otherwise auto.
