    ...
    try {
      ...
      if (mode === 'setup-token') {
        ...
        void sendNotification({
          message: 'localcode login successful',
...
function OAuthStatusMessage({
  ...
  switch (oauthStatus.state) {
    case 'idle': {
      const promptText =
        startingMessage ||
        'localcode can be used with your Claude subscription or billed based on API usage through your Console account.'
      ...
      const loginOptions = [
        ...
        },
        {
          label: (
            <Text>
              3rd-party platform ·{' '}
              <Text dimColor>OpenAI-compatible APIs, Gemini, Bedrock, and more</Text>
              {'\n'}
            </Text>
          ),
    ...
    case 'creating_api_key':
      return (
        <Box flexDirection="column" gap={1}>
          <Box>
            <Spinner />
            <Text>Creating API key for localcode…</Text>
          </Box>
        </Box>
      )
