...
import { formatCost, getTotalCost, getTotalInputTokens, getTotalOutputTokens, getTotalDuration } from 'src/cost-tracker.js';
...
async function fetchDeepSeekBalance(): Promise<BalanceInfo[] | null> {
  ...
  try {
    ...
    const data = await res.json() as { is_available?: boolean; balance_infos?: BalanceInfo[] }
    return data.balance_infos ?? null
  } catch {
    return null
  }
}

function DeepSeekUsage(): React.ReactNode {
  const model = process.env.OPENAI_MODEL ?? 'deepseek-v4-pro'
  const inputTokens = getTotalInputTokens()
  const outputTokens = getTotalOutputTokens()
  const costUSD = getTotalCost()
  const durationMs = getTotalDuration()
  const durationSec = (durationMs / 1000).toFixed(1)

  const [balance, setBalance] = useState<BalanceInfo[] | null | 'loading'>('loading')

  useEffect(() => {
    fetchDeepSeekBalance().then(setBalance)
  }, [])

  const usdBalance = Array.isArray(balance)
    ? balance.find(b => b.currency === 'USD')
    : null
        ...
        {balance === 'loading' ? (
          ...
          <>
            <Box gap={2}>
              ...
              <Text color="green">${usdBalance.total_balance} USD</Text>
            </Box>
            {parseFloat(usdBalance.granted_balance) > 0 && (
              <Box gap={2}>
                <Text dimColor>Free credits</Text>
                <Text>${usdBalance.granted_balance}</Text>
              </Box>
            )}
          </>
        ) : (
          <Text dimColor>Could not fetch balance</Text>
        )}
      </Box>
      <Text dimColor>
        <ConfigurableShortcutHint action="confirm:no" context="Settings" fallback="Esc" description="cancel" />
