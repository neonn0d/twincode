...
import { getKairosActive, getUserMsgOptIn, getTotalCostUSD } from '../bootstrap/state.js';
...
function SpinnerWithVerbInner({
  ...
  const [randomVerb, setRandomVerb] = useState(() => sample(getSpinnerVerbs()));
  useEffect(() => {
    const id = setInterval(() => setRandomVerb(sample(getSpinnerVerbs())), 3000);
...
export function BriefIdleStatus() {
  ...
  const bgText = runningCount > 0 ? `${runningCount} in background` : "";
  const rightText = [bgText, costStr].filter(Boolean).join(" \u00b7 ");
