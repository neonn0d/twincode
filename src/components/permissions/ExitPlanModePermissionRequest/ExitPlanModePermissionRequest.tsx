  ...
  if (isEmpty) {
    ...
    return <PermissionDialog color="planMode" title="Exit plan mode?" workerBadge={workerBadge}>
        <Box flexDirection="column" paddingX={1} marginTop={1}>
          <Text>twin wants to exit plan mode</Text>
          <Box marginTop={1}>
            <Select options={[{
            label: 'Yes',
            value: 'yes' as const
      ...
      <PermissionDialog color="planMode" title="Ready to code?" innerPaddingX={0} workerBadge={workerBadge}>
        ...
        overflow="hidden">
          ...
          <Box flexDirection="column" paddingX={1}>
            ...
            {!useStickyFooter && <>
                <Text dimColor>
                  twin has written up a plan and is ready to execute. Would
                  you like to proceed?
                </Text>
                <Box marginTop={1}>
...
export function buildPlanApprovalOptions({
  ...
  if (showUltraplan) {
    options.push({
      label: 'No, refine with Ultraplan on twin on the web',
      value: 'ultraplan'
    });
  }
  options.push({
    ...
    value: 'no',
    placeholder: 'Tell twin what to change',
