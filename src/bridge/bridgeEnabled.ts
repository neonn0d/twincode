...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isClaudeAISubscriber()) {
      return 'Remote Control requires a claude.ai subscription. Run `twin auth login` to sign in with your claude.ai account.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires a full-scope login token. Long-lived tokens (from `twin setup-token` or CLAUDE_CODE_OAUTH_TOKEN) are limited to inference-only for security reasons. Run `twin auth login` to use Remote Control.'
    }
    if (!getOauthAccountInfo()?.organizationUuid) {
      return 'Unable to determine your organization for Remote Control eligibility. Run `twin auth login` to refresh your account information.'
    }
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of twin (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`twin update\` to update.`
    }
  }
  return null
}
