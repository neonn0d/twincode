...
export function REPL({
  ...
  const terminalTitle = sessionTitle ?? agentTitle ?? haikuTitle ?? 'localcode';
  ...
  // That write storm is the primary trigger for ~/.localcode.json corruption
  // (GH #3117).
  ...
  useEffect(() => {
    const handleSuspend = () => {
      ...
      process.stdout.write(`\nlocalcode has been suspended. Run \`fg\` to bring localcode back.\nNote: ctrl + z now suspends localcode, ctrl + _ undoes input.\n`);
    };
    const handleResume = () => {
  ...
  const mainReturn = <KeybindingSetup>
    ...
    <MCPConnectionManager key={remountKey} dynamicMcpConfig={dynamicMcpConfig} isStrictMcpConfig={strictMcpConfig}>
      <FullscreenLayout scrollRef={scrollRef} overlay={toolPermissionOverlay} bottomFloat={isBuddyEnabled() && companionVisible && !companionNarrow ? <CompanionFloatingBubble /> : undefined} modal={centeredModal} modalScrollRef={modalScrollRef} dividerYRef={dividerYRef} hidePill={!!viewedAgentTask} hideSticky={!!viewedTeammateTask} newMessageCount={unseenDivider?.count ?? 0} onPillClick={() => {
        ...
        <Box flexGrow={1} />
        {showSpinner && !isExiting && !isShuttingDown() && <SpinnerWithVerb mode={streamMode} spinnerTip={spinnerTip} responseLengthRef={responseLengthRef} apiMetricsRef={apiMetricsRef} overrideMessage={spinnerMessage} spinnerSuffix={stopHookSpinnerSuffix} verbose={verbose} loadingStartTimeRef={loadingStartTimeRef} totalPausedMsRef={totalPausedMsRef} pauseStartTimeRef={pauseStartTimeRef} overrideColor={spinnerColor} overrideShimmerColor={spinnerShimmerColor} hasActiveTools={inProgressToolUseIDs.size > 0} leaderIsIdle={!isLoading} />}
