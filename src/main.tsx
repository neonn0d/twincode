...
export async function main() {
  ...
  if (feature('DIRECT_CONNECT')) {
    ...
    if (ccIdx !== -1 && _pendingConnect) {
      ...
      _pendingConnect.dangerouslySkipPermissions = rawCliArgs.includes('--dangerously-skip-permissions') || rawCliArgs.includes('--godmode');
      if (rawCliArgs.includes('-p') || rawCliArgs.includes('--print')) {
        ...
        const gmIdx = stripped.indexOf('--godmode');
        if (gmIdx !== -1) stripped.splice(gmIdx, 1);
  ...
  if (feature('SSH_REMOTE') && _pendingSSH) {
    ...
    if (rawCliArgs[0] === 'ssh') {
      ...
      const gmIdx = rawCliArgs.indexOf('--godmode');
      if (gmIdx !== -1) {
        _pendingSSH.dangerouslySkipPermissions = true;
        rawCliArgs.splice(gmIdx, 1);
      }
      ...
      // (which persists under the remote's ~/.localcode/projects/<cwd>/).
    ...
    if (rawCliArgs[0] === 'ssh' && rawCliArgs[1] && !rawCliArgs[1].startsWith('-')) {
      ...
      if (rest.includes('-p') || rest.includes('--print')) {
        process.stderr.write('Error: headless (-p/--print) mode is not supported with twin ssh\n');
...
async function run(): Promise<CommanderCommand> {
  ...
  program.name('twincode').description(`twincode - starts an interactive session by default, use -p/--print for non-interactive output`).argument('[prompt]', 'Your prompt', String)
  ...
  .option('--plugin-dir <path>', 'Load plugins from a directory for this session only (repeatable: --plugin-dir A --plugin-dir B)', (val: string, prev: string[]) => [...prev, val], [] as string[]).option('--disable-slash-commands', 'Disable all skills', () => true).option('--chrome', 'Enable Claude in Chrome integration').option('--no-chrome', 'Disable Claude in Chrome integration').option('--file <specs...>', 'File resources to download at startup. Format: file_id:relative_path (e.g., --file file_abc:doc.txt file_def:img.png)').action(async (prompt, options) => {
    ...
    if (prompt === 'code') {
      ...
      console.warn(chalk.yellow('Tip: You can launch twin with just `twin`'));
      prompt = undefined;
    }
    ...
    // Register PID file for concurrent-session detection (~/.localcode/sessions/)
    ...
    if (options.continue) {
      ...
      if (!targetSessionId) {
        ...
        if (sessions.length === 0) {
          ...
          return await exitWithMessage(root, `Assistant installed in ${installedDir}. The daemon is starting up — run \`twin assistant\` again in a few seconds to connect.`, {
            exitCode: 0,
      ...
      if (remote !== null) {
        ...
        if (!isRemoteTuiEnabled) {
          ...
          process.stdout.write(`Resume with: twin --teleport ${createdSession.id}\n`);
          await gracefulShutdown(0);
          process.exit(0);
        }
            ...
            if (repoValidation.status === 'mismatch' || repoValidation.status === 'not_in_repo') {
              ...
              if (sessionRepo) {
                ...
                if (existingPaths.length > 0) {
                  ...
                  throw new TeleportOperationError(`You must run twin --teleport ${teleport} from a checkout of ${sessionRepo}.`, chalk.red(`You must run twin --teleport ${teleport} from a checkout of ${chalk.bold(sessionRepo)}.\n`));
                }
              }
  ...
  }).version(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (twincode)`, '-v, --version', 'Output the version number');
  ...
  mcp.command('serve').description(`Start the twin MCP server`).option('-d, --debug', 'Enable debug mode', () => true).option('--verbose', 'Override verbose mode setting from config', () => true).action(async ({
    debug,
    verbose
  }: {
    debug?: boolean;
  ...
  if (feature('DIRECT_CONNECT')) {
    program.command('server').description('Start a twin session server').option('--port <number>', 'HTTP port', '0').option('--host <string>', 'Bind address', '0.0.0.0').option('--auth-token <token>', 'Bearer token for auth').option('--unix <path>', 'Listen on a unix domain socket').option('--workspace <dir>', 'Default working directory for sessions that do not specify cwd').option('--idle-timeout <ms>', 'Idle timeout for detached sessions in ms (0 = never expire)', '600000').option('--max-sessions <n>', 'Maximum concurrent sessions (0 = unlimited)', '32').action(async (opts: {
      ...
      if (existing) {
        process.stderr.write(`A twin server is already running (pid ${existing.pid}) at ${existing.httpUrl}\n`);
        process.exit(1);
      }
  ...
  if (feature('DIRECT_CONNECT')) {
    program.command('open <cc-url>').description('Connect to a twin server (internal — use cc:// URLs)').option('-p, --print [prompt]', 'Print mode (headless)').option('--output-format <format>', 'Output format: text, json, stream-json', 'text').action(async (ccUrl: string, opts: {
  ...
  const pluginCmd = program.command('plugin').alias('plugins').description('Manage twin plugins').configureHelp(createSortedHelpConfig());
  ...
  const marketplaceCmd = pluginCmd.command('marketplace').description('Manage twin marketplaces').configureHelp(createSortedHelpConfig());
  ...
  pluginCmd.command('uninstall <plugin>').alias('remove').alias('rm').description('Uninstall an installed plugin').option('-s, --scope <scope>', 'Uninstall from scope: user, project, or local', 'user').option('--keep-data', "Preserve the plugin's persistent data directory (~/.twincode/plugins/data/{id}/)").addOption(coworkOption()).action(async (plugin: string, options: {
    scope?: string;
    cowork?: boolean;
    keepData?: boolean;
  }) => {
  ...
  if (feature('KAIROS')) {
    program.command('assistant [sessionId]').description('Attach the REPL as a client to a running bridge session. Discovers sessions via API if no sessionId given.').action(() => {
      ...
      process.stderr.write('Usage: twin assistant [sessionId]\n\n' + 'Attach the REPL as a viewer client to a running bridge session.\n' + 'Omit sessionId to discover and pick from available sessions.\n');
      process.exit(1);
    });
  }

  // Doctor command - check installation health
  program.command('doctor').description('Check the health of your twin auto-updater. Note: The workspace trust dialog is skipped and stdio servers from .mcp.json are spawned for health checks. Only use this command in directories you trust.').action(async () => {
    const [{
      doctorHandler
    }, {
      createRoot
  ...
  // claude install
  program.command('install [target]').description('Install twin native build. Use [target] to specify version (stable, latest, or specific version)').option('--force', 'Force installation even if already installed').action(async (target: string | undefined, options: {
    force?: boolean;
  }) => {
    const {
      installHandler
