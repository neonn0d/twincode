...
;(globalThis as Record<string, unknown>).MACRO = {
  ...
  ISSUES_EXPLAINER: 'report the issue at https://github.com/neonn0d/twin/issues',
...
test('CLI identity prefixes describe localcode instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('localcode')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('localcode')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes localcode instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('localcode')
...
test('built-in agent prompts describe localcode instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('localcode')
  ...
  expect(generalPrompt).toContain('localcode')
  ...
  expect(explorePrompt).toContain('localcode')
  ...
  expect(guidePrompt).toContain('localcode')
  expect(guidePrompt).toContain('You are the localcode guide agent.')
  expect(guidePrompt).toContain('**localcode** (the CLI tool)')
