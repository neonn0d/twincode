    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      const streamingGen = queryModelWithStreaming({
        ...
        options: {
          ...
          // Use fast (non-reasoning) model for compact when available — reasoning
          // models add 30-60s overhead for a summarization task that doesn't need it.
          model: process.env.CLAUDE_CODE_USE_OPENAI === '1' && context.options.mainLoopModel.includes('pro')
            ? (process.env.OPENAI_FAST_MODEL ?? 'deepseek-v4-flash')
            : context.options.mainLoopModel,
