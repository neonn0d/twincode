...
function createTempAuthJson(payload: Record<string, unknown>): string {
  const dir = mkdtempSync(join(tmpdir(), 'localcode-codex-'))
  tempDirs.push(dir)
...
describe('Codex request translation', () => {
  ...
  test('recovers Codex web search text and sources from sparse completed response', () => {
    const output = webSearchToolTest.makeOutputFromCodexWebSearchResponse(
        ...
        output: [
          {
            ...
            sources: [
              {
                title: 'localcode repo',
                url: 'https://github.com/example/localcode',
              },
            ],
          },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'localcode is available on GitHub.',
                sources: [
                  {
                    title: 'Docs',
                    url: 'https://docs.example.com/localcode',
                  },
                ],
              },
            ],
      ...
      'localcode GitHub 2026',
      0.42,
    )

    expect(output.results).toEqual([
      'localcode is available on GitHub.',
      {
            ...
            content: [
              {
                ...
                sources: [
                  { title: 'Docs', url: 'https://docs.example.com/localcode' },
                ],
              },
            ],
          },
