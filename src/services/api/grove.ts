...
export async function checkGroveForNonInteractive(): Promise<void> {
  ...
  if (shouldShowGrove) {
    ...
    if (config === null || config.notice_is_grace_period) {
      ...
      writeToStderr(
        '\nAn update to our Consumer Terms and Privacy Policy will take effect on October 8, 2025. Run `twin` to review the updated terms.\n\n',
      )
      await markGroveNoticeViewed()
    } else {
      // Grace period has ended - show error message and exit
      writeToStderr(
        '\n[ACTION REQUIRED] An update to our Consumer Terms and Privacy Policy has taken effect on October 8, 2025. You must run `twin` to review the updated terms.\n\n',
      )
