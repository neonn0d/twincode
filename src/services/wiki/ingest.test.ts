...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'localcode-wiki-ingest-'))
  tempDirs.push(dir)
  return dir
}
...
test('ingestLocalWikiSource creates a source note and updates log/index', async () => {
  ...
  expect(result.sourceNote.startsWith('.localcode/wiki/sources/')).toBe(true)
  ...
  expect(index).toContain(result.sourceNote.replace('.localcode/wiki/', './'))
})
