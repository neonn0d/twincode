...
const NEW_INIT_PROMPT = `Set up a minimal TWIN.md (and optionally CLAUDE.local.md, skills, and hooks) for this repo. The root project instruction file is loaded into every twin session, so it must be concise — only include what Claude would get wrong without it.
...
- "Which instruction files should /init set up?"
  Options: "Project TWIN.md" | "Personal CLAUDE.local.md" | "Both project + personal"
...
If the user chose project TWIN.md or both: ask about codebase practices — non-obvious commands, gotchas, branch/PR conventions, required env setup, testing quirks. Skip things already in README or obvious from manifest files. Do not mark any options as "recommended" — this is about how their team works, not best practices.
...
If the user chose personal CLAUDE.local.md or both: ask about them, not the codebase. Do not mark any options as "recommended" — this is about their personal preferences, not best practices. Examples of questions:
  ...
  - Only if Phase 2 found multiple git worktrees: ask whether their worktrees are nested inside the main repo (e.g., \`.claude/worktrees/<name>/\`) or siblings/external (e.g., \`../myrepo-feature/\`). If nested, the upward file walk finds the main repo's CLAUDE.local.md automatically — no special handling needed. If sibling/external, the personal content should live in a home-directory file (e.g., \`~/.twin/<project-name>-instructions.md\`) and each worktree gets a one-line CLAUDE.local.md stub that imports it: \`@~/.twin/<project-name>-instructions.md\`. Never put this import in the project TWIN.md — that would check a personal reference into the team-shared file.
...
## Phase 4: Write TWIN.md (if user chose project or both)
...
**Consume \`note\` entries from the Phase 3 preference queue whose target is TWIN.md** (team-level notes) — add each as a concise line in the most relevant section. These are the behaviors the user wants Claude to follow but didn't need guaranteed (e.g., "propose a plan before implementing", "explain the tradeoffs when refactoring"). Leave personal-targeted notes for Phase 5.
...
Write a minimal CLAUDE.local.md at the project root. This file is automatically loaded alongside TWIN.md. After creating it, add \`CLAUDE.local.md\` to the project's .gitignore so it stays private.
...
If Phase 2 found multiple git worktrees and the user confirmed they use sibling/external worktrees (not nested inside the main repo): the upward file walk won't find a single CLAUDE.local.md from all worktrees. Write the actual personal content to \`~/.twin/<project-name>-instructions.md\` and make CLAUDE.local.md a one-line stub that imports it: \`@~/.twin/<project-name>-instructions.md\`. The user can copy this one-line stub to each sibling worktree. Never put this import in the project TWIN.md. If worktrees are nested inside the main repo (e.g., \`.claude/worktrees/\`), no special handling is needed — the main repo's CLAUDE.local.md is found automatically.
