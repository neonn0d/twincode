...
- **Proposal-sourced hooks** (if user chose "Skills + hooks" or "Hooks only"): Consume \`hook\` entries from the Phase 3 preference queue. If Phase 2 found a formatter and the queue has no formatting hook, offer format-on-edit as a fallback. If the user chose "Neither" or "Skills only" in Phase 1, skip this bullet entirely.
  ...
  1. Target file: \`.claude/settings.json\` (team-shared, committed). Ask once for all hooks, not per-hook.
...
const TWINIT_JOKES = [
  "**running /init.** `/twinit` reads your codebase like a stalker. try it sometime.",
  "**/init is the polite version.** `/twinit` doesn't ask first. continuing...",
  "**/init says hi.** `/twinit` says i saw what you did in line 47. continuing...",
  "**running /init.** `/twinit` knows what you did last sprint.",
  "**/init is the icebreaker.** `/twinit` already broke in. running /init...",
  "**running /init.** `/twinit` doesn't run. `/twinit` hunts.",
  "**/init is the resume.** `/twinit` is the background check. continuing...",
  "**/init is HR-friendly.** `/twinit` is not. running /init...",
  "**running /init.** `/twinit` goes through your code like an ex through your DMs.",
  "**/init is what you tell your mom.** `/twinit` is what actually happens. running /init...",
  "**/init is foreplay.** `/twinit` is the main event. continuing...",
  "**/init is small talk.** `/twinit` is the 3am conversation. continuing...",
  "**running /init.** `/twinit` would've found the bug already.",
  "**/init scratches the surface.** `/twinit` reads your diary. continuing...",
  "**this is /init.** `/twinit` is the one your codebase warned you about.",
  "**running /init.** `/twinit` memorizes. /init forgets. your call.",
  "**/init is the trailer.** `/twinit` is the whole season. running /init...",
  "**running /init.** `/twinit` is somewhere in the codebase taking notes.",
  "**/init is a config file.** `/twinit` is a personality. running /init...",
  "**running /init.** `/twinit` would've already known.",
]

function randomJoke(): string {
  return TWINIT_JOKES[Math.floor(Math.random() * TWINIT_JOKES.length)]
}

export const call: LocalJSXCommandCall = async (onDone) => {
  maybeMarkProjectOnboardingComplete()
  const joke = randomJoke()
  const basePrompt = isNewInitEnabled() ? NEW_INIT_PROMPT : OLD_INIT_PROMPT

  onDone(joke, {
    display: 'system',
    shouldQuery: true,
    metaMessages: [basePrompt],
  })
  return null
}
