...
export function registerMcpAddCommand(mcp: Command): void {
  mcp
    ...
    .description(
      'Add an MCP server to twincode.\n\n' +
        'Examples:\n' +
        '  # Add HTTP server:\n' +
        '  twincode mcp add --transport http sentry https://mcp.sentry.dev/mcp\n\n' +
        '  # Add HTTP server with headers:\n' +
        '  twincode mcp add --transport http corridor https://app.corridor.dev/api/mcp --header "Authorization: Bearer ..."\n\n' +
        '  # Add stdio server with environment variables:\n' +
        '  twincode mcp add -e API_KEY=xxx my-server -- npx my-mcp-server\n\n' +
        '  # Add stdio server with subprocess flags:\n' +
        '  twincode mcp add my-server -- my-command --some-flag arg1',
    )
    .option(
      '-s, --scope <scope>',
    ...
    .addOption(
      new Option(
        '--xaa',
        "Enable XAA (SEP-990) for this server. Requires 'twincode mcp xaa setup' first. Also requires --client-id and --client-secret (for the MCP server's AS).",
      ...
      if (!name) {
        cliError(
          'Error: Server name is required.\n' +
            'Usage: twincode mcp add <name> <command> [args...]',
        )
      ...
      try {
        ...
        if (xaa) {
          ...
          if (!getXaaIdpSettings()) {
            missing.push(
              "'twincode mcp xaa setup' (settings.xaaIdp not configured)",
            )
          }
          if (missing.length) {
        ...
        if (transport === 'sse') {
          ...
          if (!transportExplicit && looksLikeUrl) {
            ...
            process.stderr.write(
              `If this is an HTTP server, use: twincode mcp add --transport http ${name} ${actualCommand}\n`,
            )
            process.stderr.write(
              `If this is an SSE server, use: twincode mcp add --transport sse ${name} ${actualCommand}\n`,
            )
          }
