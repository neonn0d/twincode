...
export function buildProfileSaveMessage(
  ...
  if (options?.activatedInSession) {
    lines.push('localcode switched to it for this session.')
    ...
    lines.push('Restart localcode to use it.')
  }
...
function ProviderChooser({
  ...
  const helperText = canUseCodexOAuth
    ? 'Save a provider profile without editing environment variables first. Codex profiles backed by env, auth.json, or localcode secure storage can switch this session immediately when validation succeeds.'
  ...
  if (summary.savedProfileLabel !== 'none') {
    options.push({
      ...
      value: 'clear',
      description: 'Remove .localcode-profile.json and return to normal startup',
    })
  }
...
function CodexOAuthStep({
  ...
  const handleAuthenticated = React.useCallback(async (
    ...
    if (!env) {
      throw new Error(
        'Codex OAuth succeeded, but localcode could not build a Codex profile from the stored credentials.',
      )
    }
  ...
  return (
    <Dialog title="Codex OAuth" onCancel={onBack}>
      <Box flexDirection="column" gap={1}>
        <Text>
          Finish signing in with ChatGPT in your browser. localcode will store
        ...
        {status.browserOpened === false ? (
          ...
          <Text dimColor>
            Browser opened. Complete the sign-in there, then localcode will
        ...
        <ProviderChooser
          onChoose={value => {
            if (value === 'auto') {
              ...
              onDone(`Removed saved provider profile at ${filePath}. Restart localcode to go back to normal startup.`, {
                display: 'system',
              })
            } else {
