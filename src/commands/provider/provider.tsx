...
export function buildProfileSaveMessage(
  ...
  if (options?.activatedInSession) {
    lines.push('twin switched to it for this session.')
    ...
    lines.push('Restart twin to use it.')
  }
...
function ProviderChooser({
  ...
  const helperText = canUseCodexOAuth
    ? 'Save a provider profile without editing environment variables first. Codex profiles backed by env, auth.json, or twin secure storage can switch this session immediately when validation succeeds.'
...
function CodexOAuthStep({
  ...
  const handleAuthenticated = React.useCallback(async (
    ...
    if (!env) {
      throw new Error(
        'Codex OAuth succeeded, but twin could not build a Codex profile from the stored credentials.',
      )
    }
  ...
  return (
    <Dialog title="Codex OAuth" onCancel={onBack}>
      <Box flexDirection="column" gap={1}>
        <Text>
          Finish signing in with ChatGPT in your browser. twin will store
        ...
        {status.browserOpened === false ? (
          ...
          <Text dimColor>
            Browser opened. Complete the sign-in there, then twin will
        ...
        <ProviderChooser
          onChoose={value => {
            if (value === 'auto') {
              ...
              onDone(`Removed saved provider profile at ${filePath}. Restart twin to go back to normal startup.`, {
                display: 'system',
              })
            } else {
