...
function OnboardGithub(props: {
  ...
  const finalize = useCallback(
    async (
      ...
      onChangeAPIKey()
      onDone(
        'GitHub Copilot onboard complete. Copilot token and OAuth token stored in secure storage (Windows/Linux: ~/.localcode/.credentials.json, macOS: Keychain fallback to ~/.localcode/.credentials.json); user settings updated. Restart if the model does not switch.',
        { display: 'user' },
      )
    },
