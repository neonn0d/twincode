...
export function shouldSkipHookDueToTrust(): boolean {
  ...
  // When using non-Anthropic API (twin/OpenAI shim), trust dialog is never shown
  // so hooks must always be allowed
  if (process.env.CLAUDE_CODE_USE_OPENAI === '1' || process.env.OPENAI_BASE_URL) {
    return false
  }
