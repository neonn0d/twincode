...
describe('KnowledgeGraph Global Persistence & RAG', () => {
  ...
  const configDir = mkdtempSync(join(tmpdir(), 'localcode-knowledge-graph-'))
  ...
  it('deduplicates entities and updates attributes', () => {
    addGlobalEntity('tool', 'localcode', { status: 'alpha' })
    addGlobalEntity('tool', 'localcode', { status: 'beta', version: '0.6.0' })

    const graph = loadProjectGraph(cwd)
    const entities = Object.values(graph.entities).filter(e => e.name === 'localcode')
