    ...
    if (!claudeInPath && !validAlias) {
      if (existingAlias) {
        ...
        warnings.push({
          ...
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias ${getCliBinaryName()}="~/.twincode/local/${getCliBinaryName()}"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Create alias: alias ${getCliBinaryName()}="~/.twincode/local/${getCliBinaryName()}"`,
        })
      }
    }
  }
