...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    normalizedPath.endsWith(`${sep}.localcode${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.localcode${sep}settings.local.json`) ||
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const openCommandsDir = join(getOriginalCwd(), '.localcode', 'commands')
  const openAgentsDir = join(getOriginalCwd(), '.localcode', 'agents')
  const openSkillsDir = join(getOriginalCwd(), '.localcode', 'skills')
