...
beforeEach(() => {
  tempDir = mkdtempSync(join(tmpdir(), 'localcode-env-test-'))
...
test('getGlobalClaudeFile: new install returns .localcode.json when neither file exists', async () => {
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.localcode.json'))
})
...
test('getGlobalClaudeFile: migrated user uses .localcode.json when both files exist', async () => {
  writeFileSync(join(tempDir, '.claude.json'), '{}')
  writeFileSync(join(tempDir, '.localcode.json'), '{}')
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.localcode.json'))
})
